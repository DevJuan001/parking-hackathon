...
- **Vehicle type compatibility on spot assignment**: `SPOTS.vehicle_type_id` constrains which vehicles a spot can host (FK to `VEHICLE_TYPES`). When `EntriesService.create_entry` auto-assigns a spot to a plate (or a future flow assigns a spot explicitly), the spot's `vehicle_type_id` must equal the plate's `vehicle_type_id`. The service resolves the plate's `vehicle_type_id` (from the existing `PLATES` row, or from the inferred type when the plate is created on the fly) and passes it to `SpotsRepository.find_available_spot(parking_id, vehicle_type_id, connection)`, which filters by it. When no compatible spot is available, raise `ServiceError("No hay plazas disponibles para este tipo de vehículo, libera una e intentalo nuevamente")` (the repository returns the same string; the service surfaces it). Never return a spot whose type does not match the plate.
